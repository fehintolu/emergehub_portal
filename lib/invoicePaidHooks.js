...
const { deferWorkspacePromotionUntilAccess } = require('./serviceRequestPaymentPromote');
const {
  syncWorkspaceMemberPlanAfterInvoicePaid,
  resolveAccessStartWhenMarkingPaid,
} = require('./workspacePlanActivation');

/**
 * Run after an invoice is set to `paid` inside an open transaction.
 * - Workspace access window (plan duration) via applyPaidInvoiceAccessWindow when a start datetime is provided
 * - Catalogue member_plans row (active or pending_activation) for workspace bookings
 * - Linked service requests → In Progress + timeline + notify + email (deferred for paid workspace until access starts)
 * - Linked room_bookings → confirmed
 */
async function onInvoicePaid(client, invoiceId, accessStartsAt) {
  const effectiveAccessStart = await resolveAccessStartWhenMarkingPaid(client, invoiceId, accessStartsAt);
  await applyPaidInvoiceAccessWindow(client, invoiceId, effectiveAccessStart);
  await syncWorkspaceMemberPlanAfterInvoicePaid(client, invoiceId, effectiveAccessStart);
  ...
  for (const srId of srIdSet) {
    ...
    if (deferWorkspacePromotionUntilAccess(sr)) {
      continue;
    }

    if (sr.member_id) creditMembers.add(sr.member_id);
  ...
  for (const mid of creditMembers) {
    await ensureCreditLedgerAfterPaidPlan(client, mid);
  }
  await processPaidInvoiceCapacity(client, invoiceId);
