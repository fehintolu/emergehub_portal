...
/** Admin portal (staff) — separate copy and subject from member password reset. */
async function sendAdminPasswordResetEmail({ to, username, resetUrl }) {
  const html = wrapHtml(
    'Reset admin password',
    `<p>Hello ${escapeHtml(username)},</p><p>We received a request to reset your <strong>EmergeHub admin portal</strong> password. If you did not request this, ignore this email — your password will stay the same.</p><p>This link expires in about one hour.</p>`,
    resetUrl,
    'Set new admin password'
  );
  return sendMail({
    to,
    subject: 'Reset your EmergeHub admin password',
    html,
  });
}
...
/** In-app notification mirrored to email (member portal). */
async function sendMemberPortalNotificationEmail({
  to,
  name,
  title,
  message,
  ...
  const path = linkPath && String(linkPath).trim().startsWith('/') ? String(linkPath).trim() : null;
  const html = wrapHtml(
    title,
    `<p>Hi ${escapeHtml(name)},</p><p>${safeBody}</p>`,
    path ? `${portalUrl || ''}${path}` : null,
    path ? 'Open in portal' : null
  );
  return sendMail({ to, subject: title, html });
}

/** Member email when an admin sends a message (and optional attachment) on a service request. */
async function sendServiceRequestAdminMessageEmail({
  to,
  name,
  serviceName,
  messagePreview,
  ...
  const base = String(portalUrl || '').replace(/\/$/, '');
  const path = serviceRequestId ? `/services/${serviceRequestId}` : '/services';
  const attachNote = hasAttachment ? '<p><strong>An attachment</strong> was included with this message.</p>' : '';
  const preview =
    messagePreview && String(messagePreview).trim()
      ? `<p>${escapeHtml(String(messagePreview).trim()).replace(/\n/g, '<br>')}</p>`
      : '';
  const html = wrapHtml(
    'New message on your service request',
    `<p>Hi ${escapeHtml(name)},</p><p>You have a new message regarding <strong>${escapeHtml(
      serviceName
    )}</strong>.</p>${preview}${attachNote}`,
    `${base}${path}`,
    'View request'
  );
  return sendMail({
...
/** Internal: customer posted on a service request — notify managers, super admins, assigned consultant. */
async function sendServiceRequestStaffActivityEmails({
  recipients,
  subject,
  lines,
  adminOpenPath,
  ...
  const path =
    adminOpenPath && String(adminOpenPath).startsWith('/') ? String(adminOpenPath) : '/admin/service-requests';
  const inner = lines
  ...
  const uniq = [...new Set(recipients.map((e) => String(e || '').trim().toLowerCase()).filter(Boolean))];
...
module.exports = {
  ...
  sendServiceRequestStaffActivityEmails,
};
